...
nodo_t* nodo_crear(const char* clave, void* dato){
	...
	nodo->dato = lista_crear();
	lista_insertar_ultimo(nodo->dato, dato);
	nodo->izq = NULL;
	nodo->der = NULL;
	return nodo;
}
...
bool abb_guardar(abb_t* arbol, const char* clave, void* dato){
	...
	while (abb != NULL){
		if(arbol->comparador(abb->clave, clave) == 0){
			if(lista_insertar_ultimo(abb->dato, dato)) return true;
			return false;
		}
		if(arbol->comparador(abb->clave, clave) > 0){
			padre = abb;
...
void** todos_los_valores(abb_t* arbol){
	void** nodos_recorridos = malloc(sizeof(void*) * N);
	int i = 0;
	int tam_arr = N;
	lista_iter_t* lista_iter = lista_iter_crear(abb->dato);
	while (!lista_iter_al_final(lista_iter)){
		nodos_recorridos[i] = lista_iter_ver_actual(lista_iter);
		lista_iter_avanzar(lista_iter);
		i++;
		if ( i == tam_arr){
			nodos_recorridos = realloc(nodos_recorridos, tam_arr * RED * sizeof(void*));
			tam_arr = tam_arr * RED;
		}
	}
...
void** abb_obtener(const abb_t* arbol, const char* clave){
	...
	void** datos = todos_los_valores(abb);
	return datos;
}
...
void** abb_busc_iter_corte(abb_t* arbol, const char* clave_busc, const char* clave_corte){
	...
	while (!abb_iter_in_al_final(iter) && arbol->comparador(abb_iter_in_ver_actual(iter), clave_corte) != 0){
		lista_iter_t* lista_iter = lista_iter_crear(iter->act->dato);
		while (!lista_iter_al_final(lista_iter)){
			nodos_recorridos[i] = lista_iter_ver_actual(lista_iter);
			 lista_iter_avanzar(lista_iter);
			 i++;
